...
**At-most-once delivery.** Leave `task_acks_late` off, so Celery acknowledges a task when the worker receives it. A worker that dies mid-job has already acknowledged, so the broker does not redeliver and the job stays `running` until the user acts. `worker_prefetch_multiplier` is 1, so a worker reserves only the job it runs. A surplus of jobs over workers waits in the broker queue, not in worker memory, so a worker death loses at most the one job it was running, never a buffer of queued ones, and any live worker drains the queue.
