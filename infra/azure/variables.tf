variable "subscription_id" {
  description = "Azure subscription UUID used explicitly for plan and apply."
  type        = string
...
variable "location" {
  description = "Azure Region for the baseline deployment."
  type        = string
  default     = "polandcentral"
}

variable "ubuntu_image_version" {
  description = "Pinned Canonical Ubuntu 24.04 AMD64 Marketplace image version."
  type        = string
  default     = "24.04.202607140"

  validation {
    condition     = can(regex("^24[.]04[.][0-9]+$", var.ubuntu_image_version))
    error_message = "ubuntu_image_version must be a concrete Ubuntu 24.04 image version."
  }
}

variable "vm_size" {
  description = "Linux AMD64 Azure VM size for the complete Compose topology."
  type        = string
  default     = "Standard_B2as_v2"
}

variable "os_disk_size_gib" {
  description = "Standard SSD OS-disk size in GiB."
  type        = number
  default     = 32

  validation {
    condition = (
      var.os_disk_size_gib >= 32 &&
      floor(var.os_disk_size_gib) == var.os_disk_size_gib
    )
    error_message = "os_disk_size_gib must be an integer of at least 32 GiB."
  }
}
...
variable "ssh_public_key" {
  description = "Ed25519 public key registered for the Ubuntu Azure user."
  type        = string
